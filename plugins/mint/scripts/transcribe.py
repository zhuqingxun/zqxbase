...
说明:
    ...
    - 需要设置 DASHSCOPE_API_KEY 和 OSS 凭据（环境变量或 Bitwarden）
...
# requests / oss2 / dashscope 一律在使用处懒加载，使凭据解析层可仅依赖 stdlib 导入与单测
...
# --- 凭据来源配置 ----------------------------------------------------------
# 把 Bitwarden 条目名 / CLI 路径从业务函数里提为可被环境变量覆盖的默认值，
# 使凭据层与 ASR 业务层解耦：跨用户/跨机器可移植（改 env 即可），且可独立单测
# （设 DASHSCOPE_API_KEY / OSS_* 即短路返回，无需触发真实 vault / 网络）。
DEFAULT_BW_DASHSCOPE_ITEM = '阿里bailian (标准 DashScope)'
DEFAULT_BW_OSS_ITEM = '阿里云 OSS'
DEFAULT_BW_PATH = '~/AppData/Roaming/npm/bw.cmd'


def _resolve_bw_path() -> str:
    """定位 Bitwarden CLI 可执行文件。

    优先 MINT_BW_PATH（或默认 Windows npm 路径）若存在；否则回退 PATH 查找。
    在调用时读取环境变量，便于测试注入。
    """
    import shutil
    configured = os.path.expanduser(os.getenv('MINT_BW_PATH') or DEFAULT_BW_PATH)
    if os.path.exists(configured):
        return configured
    return shutil.which('bw') or shutil.which('bw.cmd') or 'bw'


def _bw_dashscope_item() -> str:
    return os.getenv('MINT_BW_DASHSCOPE_ITEM') or DEFAULT_BW_DASHSCOPE_ITEM


def _bw_oss_item() -> str:
    return os.getenv('MINT_BW_OSS_ITEM') or DEFAULT_BW_OSS_ITEM
...
def get_dashscope_key() -> str:
    ...
    try:
        ...
        result = subprocess.run(
            [_resolve_bw_path(), 'get', 'password', _bw_dashscope_item()],
    ...
    raise RuntimeError(
        ...
        '│                                                     │\n'
        '│  mint:transcribe 需要百炼平台 API Key 才能工作。      │\n'
        '│                                                     │\n'
        '│  快速配置（推荐，永久生效）:                           │\n'
        '│    在 Claude Code 中执行:                            │\n'
        '│    /update-config set DASHSCOPE_API_KEY=sk-...       │\n'
        '│                                                     │\n'
        '│  临时配置（仅本次会话）:                               │\n'
...
def get_oss_credentials() -> tuple[str, str]:
    ...
    try:
        import subprocess
        result = subprocess.run(
            [_resolve_bw_path(), 'get', 'item', _bw_oss_item()],
            capture_output=True, text=True, timeout=10,
        )
        if result.returncode == 0:
            ...
            for f in item.get('fields', []):
                if f['name'] == 'AccessKey ID':
                    ak = f['value']
                elif f['name'] == 'AccessKey Secret':
                    sk = f['value']
            if ak and sk:
                return ak, sk
    except Exception:
...
def upload_to_oss(file_path: str) -> tuple[str, str]:
    ...
    使用 resumable_upload 分片上传 + 断点续传，适合大文件和不稳定网络。
    外层 5 次重试：网络中断后 ResumableStore 会自动续传已上传的分片。
    """
    import oss2
    ...
    bucket = oss2.Bucket(auth, OSS_ENDPOINT, OSS_BUCKET, connect_timeout=300)

    file_name = os.path.basename(file_path)
    file_size_mb = os.path.getsize(file_path) / 1024 / 1024
    oss_key = f'temp/{int(time.time())}_{file_name}'
    store = oss2.ResumableStore(root=os.path.expanduser('~/.mint_oss_upload'))
    ...
    def _progress(consumed, total):
        if total > 0:
            ...
            if pct >= last_logged_pct[0] + 5:
                last_logged_pct[0] = pct - (pct % 5)
                logger.info(f'  上传进度: {pct}% ({consumed/1024/1024:.1f}/{total/1024/1024:.1f}MB)')
    ...
    for attempt in range(1, max_outer_retries + 1):
        try:
            ...
            break
        except (oss2.exceptions.RequestError, Exception) as e:
            if attempt < max_outer_retries:
                wait = 5 * attempt
                logger.warning(f'上传中断 (第 {attempt}/{max_outer_retries} 次)，{wait}s 后断点续传: {type(e).__name__}')
                time.sleep(wait)
            else:
                raise
