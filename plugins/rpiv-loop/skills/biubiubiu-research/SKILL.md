---
name: rpiv-loop:biubiubiu-research
description: >-
  一键启动全自主 agent 团队，自动完成从信息源发现到部署的完整研究流程。brainstorm 完成后使用此命令，无需人工介入。当用户提到'自动研究'、'全自主研究'、'research biubiubiu'、'启动研究团队'、'深度调研'时触发。也适用于用户说'帮我研究一下 XXX'且研究范围足够大（需要多源调研+写作+部署）的场景。
...
**模式 A：站点项目**（当前目录有 `site/mkdocs.yml`）：
```
{topic}/
...
3. **归档过程文件**：将 `rpiv/` 下的过程文件归档到 `rpiv/archive/`
4. **向用户报告**：输出产物清单和 NotebookLM notebook ID
...
- NotebookLM notebook ID 必须在交付报告中输出，供后续使用
