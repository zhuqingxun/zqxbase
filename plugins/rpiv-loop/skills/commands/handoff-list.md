...
```bash
rpiv_loop_root="${RPIV_LOOP_ROOT:-${CLAUDE_PLUGIN_ROOT:?set RPIV_LOOP_ROOT or CLAUDE_PLUGIN_ROOT}}"
uv run --no-project python "$rpiv_loop_root/tools/list_handoffs.py" $ARGUMENTS
```
