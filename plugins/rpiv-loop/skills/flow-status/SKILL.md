...
```bash
uv run --no-project python ${CLAUDE_PLUGIN_ROOT}/tools/flow_status.py $ARGUMENTS
```
...
2. 若 stdout 含 `__NEED_LLM__` 单行标记,从该行起进入"指令模式":下一行是 JSON payload,再下面的 `PROTOCOL:` 段是给你的执行指令(协议由脚本运行时下发,SKILL 不重复维护)。按 PROTOCOL 段执行,完成后重跑 `uv run --no-project python ${CLAUDE_PLUGIN_ROOT}/tools/flow_status.py check` 输出最终一致性结果。
