...
argument-hint: [功能名称或需求描述，也可传入已有需求文档路径]
---
...
- **阶段 1**：读取需求摘要文件 `{brainstorm-summary-path}`，将其 `status` 更新为 `in-progress`。按照 RPIV create-prd 规范创建 PRD → `rpiv/requirements/prd-{feature-name}.md`。PRD 创建完成后，将需求摘要的 `status` 更新为 `completed`。PRD 规范参考读取 `~/.claude/commands/rpiv_loop/create-prd.md`
- **阶段 2**：等待 tech-research 任务完成（通过 TaskList 检查），然后按照 RPIV plan-feature 规范创建实施计划 → `rpiv/plans/plan-{feature-name}.md`。计划规范参考读取 `~/.claude/commands/rpiv_loop/plan-feature.md`。代码库分析要做充分，计划要足够详细，让 Dev agent 无需额外调研就能实现
...
- **阶段 4**：运行测试 + 代码审查。测试全部通过后，将 test-strategy 和 test-specs 的 status 更新为 `completed`。代码审查规范参考读取 `~/.claude/commands/rpiv_loop/validation/code-review.md`。审查报告保存到 `rpiv/validation/code-review-{feature-name}.md`
...
- 按照计划中的逐步任务实现代码。执行规范参考读取 `~/.claude/commands/rpiv_loop/execute.md`
