---
description: 对指定目录/模块进行全量代码审计（不依赖 git diff）。支持 4 个维度的并行审查，置信度评分过滤误报，高严重度问题自动评估爆炸半径。
---
...
启动 **4 个并行 Agent**，每个 Agent 接收完整文件清单和项目上下文，各自独立完整阅读每个文件后审查：
...
组合模式（如 `logic,security`）：启动 **2 个并行 Agent**，每个执行对应维度的深度模式。
...
- 每个维度独立评分 0-100：
  ...
  - 每个 low 扣 2 分
  - 最低 0 分
- 总分 = 各维度评分的加权平均（全量模式 4 维度均等权重；单项模式只有该维度）
